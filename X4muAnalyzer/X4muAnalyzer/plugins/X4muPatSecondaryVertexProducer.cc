 ...
 * 2024-10
 * X4muPatSecondaryVertexProducer loop reco::Muon
...
void X4muPatSecondaryVertexProducer::produce(edm::Event &iEvent, edm::EventSetup const &iSetup)
{
   ...
   if (muons->size() < 4) // 4
   {
      std::cout << "reco::Track size is too small: " << muons->size() << std::endl;
      return;
   }

   // Loop 4 Muons
   for (pat::MuonCollection::const_iterator iMuon1 = muons->begin(); iMuon1 != muons->end(); ++iMuon1)
   {
      for (pat::MuonCollection::const_iterator iMuon2 = iMuon1 + 1; iMuon2 != muons->end(); ++iMuon2)
      {
         for (pat::MuonCollection::const_iterator iMuon3 = iMuon2 + 1; iMuon3 != muons->end(); ++iMuon3)
         {
            for (pat::MuonCollection::const_iterator iMuon4 = iMuon3 + 1; iMuon4 != muons->end(); ++iMuon4)
            {  
               if(iMuon1->track().isNull() || iMuon2->track().isNull() || iMuon3->track().isNull() || iMuon4->track().isNull()) continue;
               ...
               if(muTrack1.isNull() || muTrack2.isNull() || muTrack3.isNull() || muTrack4.isNull()) continue;
               ...
               if (!muonTT1.isValid() || !muonTT2.isValid() || !muonTT3.isValid() || !muonTT4.isValid()) continue;
